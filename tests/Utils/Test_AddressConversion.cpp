...
BOOST_AUTO_TEST_CASE(testAddrBech32Decode) {
  ...
  for (auto& pair : testPairs) {
    ...
    BOOST_CHECK_MESSAGE(retCode == AddressConversionCode::OK, "Bech32 unable to decode");
...
BOOST_AUTO_TEST_CASE(testAddrDecodeNegativeCase1) {
  ...
  std::vector<string> testStrings{"zil", "z", "asdc", "1234567890abcdef1234567890abcdef1234567890abcdef"};

  for (const auto& input : testStrings) {
    Address retAddr;
    auto retCode = ToBase16Addr(input, retAddr);
    BOOST_CHECK_MESSAGE(retCode == AddressConversionCode::WRONG_ADDR_SIZE, "Address decode did not detect invalid size");
  }
}
...
BOOST_AUTO_TEST_CASE(testAddrDecodeNegativeCase2) {
  ...
  std::vector<string> testStrings{"zil1", "zil1abc", "zil1T413131515AWMRSVVEHN8U5FM0AAWSG89DY25JA46NDSRHQ"};

  for (const auto& input : testStrings) {
    Address retAddr;
    auto retCode = ToBase16Addr(input, retAddr);
    BOOST_CHECK_MESSAGE(retCode == AddressConversionCode::INVALID_BECH32_ADDR, "Bech32 decode did not detect invalid address");
  }
}
...
BOOST_AUTO_TEST_CASE(testAddrDecodeNegativeCase3) {
  ...
  for (const auto& input : testStrings) {
    Address retAddr;
    auto retCode = ToBase16Addr(input, retAddr);
    BOOST_CHECK_MESSAGE(retCode == AddressConversionCode::INVALID_ADDR, "Address decode did not detect invalid address");
  }
}
